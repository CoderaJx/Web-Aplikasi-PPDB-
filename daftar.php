...
$b = mysqli_query($koneksi,"SELECT max(nis) FROM pendaftaran");

$mek = mysqli_fetch_array($b);

if ($mek) {
  $nilai = substr($mek[0], 1);
  $mek2 = (int) $nilai;

  $mek = $mek2 + 1;
  $muncul = 'B' .str_pad($mek, 4, "0", STR_PAD_LEFT);
}else{
  $muncul = "B0001";
}
...
<div style="background: purple; color : yellow; font-size: 25px; font-weight: bold; letter-spacing: 0,5em;"><h2 align="center">HALAMAN DAFTAR</h2>
</div>
<br>
  ...
  <dir class="card card-warning">
    ...
    <form action="input_data.php" method="post">
     <div class="card-body">
        ...
        <input type="text" class="form-control" name="nis"  value="<?php echo $muncul; ?>">
         </div> 
        ...
        <input type="text" class="form-control" name="no_ayah">
        </div>
